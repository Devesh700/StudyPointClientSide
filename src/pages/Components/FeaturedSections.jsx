...
const FeaturedSections = () => {
  ...
  let length=parseInt(StudyMaterial?.length/3);
  let data=[StudyMaterial?.slice(0,length),StudyMaterial?.slice(length,length+length),StudyMaterial?.slice(length+length,StudyMaterial?.length)];
  ...
  return (
    <div className=" container mx-auto p-4 pt-10 lg:w-10/12 md:w-11/12 sm:w-full  my-4">
      ...
      >
        ...
        <Link to="/articles" className="text-orange-300 text-base hover:underline items-center gap-4 flex">Explore <FaLink/></Link>
        </div>
      </div>
            ))}
            </div>
